...
function initializeProfilePictures() {
  ...
  const profileContainers = document.querySelectorAll(
    ".profile-picture, .profile-pic, .user-avatar, .chat-avatar, .message-avatar, .conversation-avatar, .profile-picture-preview",
  );

  profileContainers.forEach((container) => {
    const img = container.querySelector("img");
    const username =
      container.getAttribute("data-username") || container.textContent.trim();
