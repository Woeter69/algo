...
function initializeSettingsNavigation() {
    ...
    if (hash) {
        const targetNav = document.querySelector(`[data-section="${hash}"]`) as HTMLElement;
        if (targetNav) {
            targetNav.click();
        }
    }
...
async function handlePrivacySubmit(e: Event) {
    e.preventDefault();
...
async function handleNotificationsSubmit(e: Event) {
    e.preventDefault();
