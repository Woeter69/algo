...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (clearFiltersBtn) {
        clearFiltersBtn.addEventListener('click', function() {
            ...
            document.querySelectorAll<HTMLSelectElement>('.filters-section select').forEach(select => {
                select.value = '';
            });
            // Reset search input
    ...
    function applyFilters(): void {
        const university = (document.getElementById('universityFilter') as HTMLSelectElement)?.value || '';
    ...
    if (connectionForm) {
        connectionForm.addEventListener('submit', async function(e) {
            ...
            if (selectedPerson) {
                const message = (document.getElementById('connectionMessage') as HTMLTextAreaElement)?.value || '';
    ...
    (window as WindowWithConnectData).connectWithPerson = function(personId: number): void {
    ...
    (window as WindowWithConnectData).viewProfile = function(personId: number): void {
        const person = currentPeople.find(p => p.id === personId);
        if (person) {
    ...
    (window as WindowWithConnectData).acceptConnection = async function(personId: number): Promise<void> {
        try {
    ...
    (window as WindowWithConnectData).declineConnection = async function(personId: number): Promise<void> {
        try {
            const response = await fetch('/api/respond_connection_request', {
                method: 'POST',
                headers: {
