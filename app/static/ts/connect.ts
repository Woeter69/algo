// Connect page TypeScript functionality - Enhanced Search & Filtering
interface Person {
  id: number;
  name: string;
  title: string;
...
interface WindowWithConnectData extends Window {
  connectData?: ConnectData;
  connectWithPerson?: (personId: number) => void;
  viewProfile?: (personId: number) => void;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const windowWithData = window as WindowWithConnectData;
  const connectData: ConnectData = windowWithData.connectData || {
    people: [],
    universities: [],
    graduationYears: [],
    locations: [],
  ...
  const processedPeople: Person[] = allPeople.map((person: any) => ({
    id: person.id,
    name: person.name,
    title: person.title,
    ...
    connectionStatus:
      person.connection_status || person.connectionStatus || "none",
    connection_id: person.connection_id || null,
    isRecommended: Math.random() > 0.7, // Random recommendation for demo
  }));

  let currentPeople: Person[] = [...processedPeople];
  let displayedCount: number = 0;
  const itemsPerLoad: number = 6;
  ...
  function initializeUserDropdown(): void {
  ...
  if (clearFiltersBtn) {
    clearFiltersBtn.addEventListener("click", function () {
      ...
      document
        .querySelectorAll<HTMLSelectElement>(".filters-section select")
        .forEach((select) => {
          select.value = "";
        });
      // Reset search input
  ...
  (window as WindowWithConnectData).clearSearch = function (): void {
    if (searchInput) {
      searchInput.value = "";
      performSearch();
      hideSearchResults();
  ...
  function applyFilters(): void {
    const university =
      (document.getElementById("universityFilter") as HTMLSelectElement)
        ?.value || "";
    const year =
      (document.getElementById("yearFilter") as HTMLSelectElement)?.value || "";
    const role =
      (document.getElementById("roleFilter") as HTMLSelectElement)?.value || "";
    const location =
    ...
    currentPeople = processedPeople.filter((person) => {
      const matchesUniversity =
        !university ||
        person.university.toLowerCase().includes(university.toLowerCase());
      const matchesYear = !year || person.graduationYear.toString() === year;
      const matchesRole =
        !role || person.role.toLowerCase() === role.toLowerCase();
  ...
  if (connectionForm) {
    connectionForm.addEventListener("submit", async function (e) {
      ...
      if (selectedPerson) {
        const message =
          (document.getElementById("connectionMessage") as HTMLTextAreaElement)
            ?.value || "";
  ...
  function renderPeople() {
    ...
    const recommendedPeople = currentPeople.filter(
      (person) => person.isRecommended,
    );
    ...
    if (loadMoreBtn) {
      loadMoreBtn.style.display =
        displayedCount < allPeople.length ? "inline-block" : "none";
    }
  }
  ...
  (window as WindowWithConnectData).connectWithPerson = function (
    personId: number,
  ): void {
  ...
  (window as WindowWithConnectData).viewProfile = function (
    personId: number,
  ): void {
    const person = currentPeople.find((p) => p.id === personId);
    if (person) {
  ...
  (window as WindowWithConnectData).acceptConnection = async function (
    personId: number,
  ): Promise<void> {
    try {
      ...
      if (result.success) {
        ...
        showNotification(
          result.message || "Failed to accept connection",
          "error",
        );
      }
    } catch (error) {
  ...
  (window as WindowWithConnectData).declineConnection = async function (
    personId: number,
  ): Promise<void> {
    try {
      const response = await fetch("/api/respond_connection_request", {
      ...
      if (result.success) {
        ...
        showNotification(
          result.message || "Failed to decline connection",
          "error",
        );
      }
    } catch (error) {
  ...
  // Navigation links
  const navLinksElements = document.querySelectorAll(
