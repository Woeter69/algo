...
interface GoWebSocketClient {
    connect(userId: string, username: string, pfpPath: string): void;
    connected: boolean;
    isConnected(): boolean;
    on(event: string, handler: (data: any) => void): void;
    send(type: string, data: any): boolean;
    sendMessage(content: string, channelId: string | number): boolean;
    joinChannel(channelId: string | number): boolean;
    leaveChannel(channelId: string | number): boolean;
    startTyping(channelId: string | number): boolean;
    stopTyping(channelId: string | number): boolean;
}
...
let currentChannelId: string | number | null = null;
let socket: GoWebSocketClient | null = null;
let processedMessages: Set<string> = new Set();
let lastSentMessageTime: number = 0;

// DOM elements
let channels: NodeListOf<Element>;
let currentChannelName: HTMLElement | null;
let currentChannelIcon: HTMLElement | null;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const channelsData: ChannelsData = (window as any).channelsData || {};
    currentUserId = (window as any).currentUserId;
    currentUsername = (window as any).currentUsername || 'User';
    currentUserPfp = (window as any).currentUserPfp || '';
    ...
    if (socket && currentUserId) {
        ...
        socket.on('connect', () => {
            ...
            setTimeout(() => {
                const firstChannel = document.querySelector('.channel') as HTMLElement;
    ...
    channels.forEach(channel => {
        channel.addEventListener('click', () => {
            ...
            if (currentChannelIcon) currentChannelIcon.innerHTML = `<i class="${channelIcon}"></i>`;
...
// Helper functions
function createMessageElement(content: string, options: {
    username?: string;
    pfp_path?: string;
    timestamp?: string;
    optimistic?: boolean;
...
function displayChannelMessage(data: MessageData): void {
...
function showUserJoinedMessage(data: any): void {
    if (!chatMessages) return;
...
function showUserLeftMessage(data: any): void {
    if (!chatMessages) return;
...
function displayMessageHistory(data: any): void {
    if (!chatMessages) return;
    ...
    const messages = data.data?.messages || data.Data?.messages || data.messages || [];
