...
def forgot_password():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            cur.execute(
                "SELECT user_id, firstname, lastname, email FROM users WHERE email = %s OR username = %s",
                (email, email),
            )
            user = cur.fetchone()
...
def contact():
    if request.method == "POST":
        ...
        try:
            ...
            cur = mydb.cursor()
            cur.execute(
                "INSERT INTO contacts (full_name, email, phone, subject_, message_)VALUES (%s, %s, %s, %s, %s)",
...
def profile(username):
    ...
    try:
        ...
        return render_template(
            ...
            user_phone=None,
        )  # TODO: Add phone field to database
