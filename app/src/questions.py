"""
Simple FAQ system for Algo platform with predefined questions and answers.
"""
...
def questions(user_qs, count):
    """Check if question exists in database and return random answer, otherwise prompt for rephrase."""
    question = {
        "What is Algo?": [
            "Algo is a platform designed to strengthen the connection between alumni and their institutions. Institutions can use it to maintain communication with their alumni for events like alumni meets, donation drives, career counseling, and more. It also enables current students to connect with their seniors for guidance and mentorship.",
        ...
        "How can I login to Algo?": [
            "To access Algo, go to the login page and enter your username and password. New users must register first by opening the sign-up page and filling in the necessary details such as full name and enrollment number. Once registered, you can log in with your new credentials.",
...
def main(count):
    """Handle user input with different prompts based on attempt count."""
    if count == 0:
        user_qs = input("Hey, How can I help you? ")
    else:
        user_qs = input(
            "Sorry, I don't have an answer for that question. Can you please rephrase that, or ask another question? "
        )
