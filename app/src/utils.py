...
def create_password_reset_email_content(
    reset_token: str, user_name: str = None, to_email: str = None
):
...
def send_password_reset_email(
    ...
    try:
        ...
        # Get email content
        subject, text_body = create_password_reset_email_content(
            reset_token, user_name, to_email
        )
        msg["Subject"] = subject
...
def send_password_changed_notification(to_email: str, user_name: str = None) -> bool:
    ...
    try:
        ...
        logger.error(
            f"Failed to send password changed notification to {to_email}: {str(e)}"
        )
        return False
