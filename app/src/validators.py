...
EMAIL_USER = os.getenv(
    "EMAIL_USER"
)  # fetching/setting/using/getting environment variable values.
...
def send_verification_email(to_email, link):
    ...
    )  # Here we are making what message to send. And related metadata like subject, from, to etc.
    ...
    with smtplib.SMTP(
        "smtp.gmail.com", 587
    ) as server:  # Starting the server at 587(standard) and actually sending the message
        server.starttls()  # Ever heard of TLS(Transport layer security) handshake. Yep that's what's happening here.
        server.login(
            EMAIL_USER, EMAIL_PASS
        )  # using credentials to login in and then send the message
        server.send_message(
            msg
        )  # using with is good because we don't have to manually close the connectoin.
...
ALLOWED_EXTENSIONS = {
    ...
    "jpeg",
    "gif",
}  # defining what extensions are allowed.
...
def login_required(f):
    @wraps(
        f
    )  # Standard Shit we use to let flask know that login_required can be used as a decorator for other function. & Keeps the original function's (f) name, docstring, and metadata intact.
    def decorated_function(
        *args, **kwargs
        ...
        return f(
            *args, **kwargs
        )  # else if they are indeed logged in call the function 'f'
...
def validate_username(username):
    if (
        not username or len(username) < 3
    ):  # checks if the username is empty or less than 3 characters long
        return False, "Username must be at least 3 characters long"
    if len(username) > 20:
        return False, "Username must be less than 20 characters"  # self explanatory
    if (
        not username.replace("_", "").replace("-", "").isalnum()
    ):  # checks if the username is alphanumerical when _ and - are removed from it.
        return (
            False,
...
def validate_email(
    email,
):  # using regular expressions (regex) for for checking proper email format
    ...
    if not re.match(
        pattern, email
    ):  # if the email doensn't match our pattern return False and Invalid email format
        return (
            False,
            "Invalid email format",
        )  # if it does match our pattern, then Hoorayy!!. return True and say that the email format is valid
...
def validate_password(password):
    if (
        not password or len(password) < 8
    ):  # simple, again checking if the password sting is empty or less than 8 characters long
    ...
    )  # these check if atleast 1 character exist that is uppercase, lower, and digit in password
    has_lower = any(c.islower() for c in password)  # returns a boolean value off course
    has_digit = any(
        c.isdigit() for c in password
    )  # note that we could have also used regex - regular expressions for this as well.

    if not (
        has_upper and has_lower and has_digit
    ):  # throws error if requirements of password not met
        return (
            False,
...
def validate_file_size(file, max_size_mb=5):
    if file:  # if a non empty file is uploaded then this statement doensn't run
        ...
        size = (
            file.tell()
        )  # get current position = file size in bytes. super smart way.
        file.seek(
            0
        )  # reset pointer back to start (important so file isn't "consumed"). i.e it can be read normally later by other functions.

        max_size_bytes = (
            max_size_mb * 1024 * 1024
        )  # converting 5mb to it's byte equivalent. love that we've used 1024 instead of 1000.
        if size > max_size_bytes:
            return (
                False,
                f"File size must be less than {max_size_mb}MB",
            )  # if file size is bigger than 5 mb throw an error.

    return (
        True,
        "Valid file size",
    )  # Also note that for using the file we learn't in python that we have to open and close it manually. here we have used the seek and tell functions without opening the file first.
